...
class GPUDisplayHelper:
    ...
    def create_display_name(gpu_info: GPUInfo, index: int) -> str:
        """创建GPU的显示名称"""
        return (
            f"{gpu_info.name} (GPU {index})" if gpu_info.pci_bus_id else gpu_info.name
        )
...
class GPUManager:
    ...
    def __add_gpu_to_list(self, gpu_info: GPUInfo):
        if gpu_info.gpu_type == GPUType.NVIDIA:
            ...
            gpu_index = (
                self.get_gpu_index_from_pci(gpu_info.pci_bus_id)
                if gpu_info.pci_bus_id
                else len(self.nvidia_gpus)
            )
            display_name = GPUDisplayHelper.create_display_name(gpu_info, gpu_index)
            self.nvidia_gpus.append(display_name)
            if DEBUG_BUILD:
                logging.debug(
                    f"添加NVIDIA GPU到列表: {display_name}, 索引: {gpu_index}, PCI总线ID: {gpu_info.pci_bus_id}"
                )
        ...
        elif gpu_info.gpu_type == GPUType.APPLE:
            self.apple_gpus.append(gpu_info.name)
            if DEBUG_BUILD:
                logging.debug(f"添加Apple GPU到列表: {gpu_info.name}")
    ...
    def __universal_detect_nvidia_gpu(self):
        ...
        for gpu_info in nvidia_gpu_info:
            ...
            if DEBUG_BUILD:
                gpu_mem_gib = (
                    BytesToGiB(gpu_info.dedicated_gpu_memory)
                    if gpu_info.dedicated_gpu_memory
                    else 0
                )
                avail_mem_gib = (
                    BytesToGiB(gpu_info.avail_dedicated_gpu_memory)
                    if gpu_info.avail_dedicated_gpu_memory
                    else 0
                )
                logging.debug(
                    f"NVIDIA GPU详情: 名称={gpu_info.name}, 索引={gpu_index}, PCI={gpu_info.pci_bus_id}, "
    ...
    def detect_gpus(self):
        ...
        if platform.system() == "Windows":
            ...
            self.detect_gpus_macos()
        else:
            logging.warning(f"当前平台不支持GPU检测: {platform.system()}")

        if DEBUG_BUILD:
            logging.debug(
                f"GPU检测完成, 共检测到: {len(self.nvidia_gpus)} 个NVIDIA GPU, "
                f"{len(self.amd_gpus)} 个AMD GPU, {len(self.intel_gpus)} 个Intel GPU, "
                f"{len(self.apple_gpus)} 个Apple GPU"
            )
            logging.debug(
                f"GPU信息映射表: {json.dumps({k: str(v.__dict__) for k, v in self.gpu_info_map.items()}, indent=2)}"
            )

    def detect_gpus_macos(self):
        if DEBUG_BUILD:
            logging.debug("在macOS平台上检测GPU")

        if platform.machine().lower() != "arm64":
            logging.info("当前不是Apple Silicon，跳过MPS检测")
            return
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                display_data = payload.get("SPDisplaysDataType", [])
                if display_data and isinstance(display_data[0], dict):
                    chip_name = (
                        display_data[0].get("sppci_model")
                        or display_data[0].get("_name")
                        or chip_name
                    )
        except Exception as e:
            logging.info(f"读取macOS GPU信息失败，使用默认名称: {e}")

        gpu_info = GPUInfo(
            index=0,
            name=f"{chip_name} (MPS)",
            gpu_type=GPUType.APPLE,
            dedicated_gpu_memory=None,
            avail_dedicated_gpu_memory=None,
            pci_bus_id="apple-mps",
        )
        self.gpu_info_map[gpu_info.pci_bus_id] = gpu_info
        self.apple_gpus.append(gpu_info.name)
        logging.info(f"检测到Apple GPU: {gpu_info.name}")
    ...
    def detect_gpus_windows(self):
        ...
        try:
            ...
            if DEBUG_BUILD:
                logging.debug(
                    f"从Windows注册表检测到 {len(adapter_values)} 个GPU适配器"
                )
                for i, adapter in enumerate(adapter_values):
                    mem_gib = BytesToGiB(adapter.MemorySize)
                    logging.debug(
                        f"适配器 {i}: 名称={adapter.AdapterString}, 显存={mem_gib:.2f}GiB, PCI={adapter.pci_bus_id}"
                    )
            ...
            for adapter in adapter_values:
                ...
                if gpu_key not in self.gpu_info_map:
                    gpu_info = GPUInfo(
                        ...
                        pci_bus_id=adapter.pci_bus_id
                        if hasattr(adapter, "pci_bus_id")
                        else None,
                    )
        ...
        try:
            ...
            for gpu in c.Win32_VideoController():
                if "AMD" in gpu.Name or "ATI" in gpu.Name:
                    ...
                    if DEBUG_BUILD:
                        logging.debug(
                            f"从WMI检测到AMD GPU: {gpu.Name}, 驱动版本: {gpu.DriverVersion}, 状态: {gpu.Status}"
                        )
    ...
    def check_gpu_ability(
        ...
        if DEBUG_BUILD:
            logging.debug(
                f"检查GPU能力: 显示名称={gpu_display_name}, 模型={model_name}, "
        ...
        if not gpu_key or gpu_key not in self.gpu_info_map:
            logging.error(
                f"未找到GPU: {gpu_display_name}, 当前已知GPU: {list(self.gpu_info_map.keys())}"
            )
        ...
        if DEBUG_BUILD:
            logging.debug(
                f"找到匹配的GPU: {gpu_key}, 类型: {gpu_info.gpu_type}, "
                f"总显存: {BytesToGiB(gpu_info.dedicated_gpu_memory):.2f}GiB"
            )

        if gpu_info.gpu_type == GPUType.APPLE:
            # Apple MPS 使用统一内存，当前无法准确读取可用显存，跳过显存阈值检查
            return GPUAbility(is_capable=True, reason="")

        if gpu_info.gpu_type not in [GPUType.NVIDIA, GPUType.AMD]:
            return GPUAbility(
                is_capable=False, reason=f"目前只支持 NVIDIA、AMD 和 Apple MPS"
            )

        if gpu_info.avail_dedicated_gpu_memory is not None:
            ability = self._check_dynamic_memory(
                gpu_info, model_name, context_length, n_parallel
            )
        else:
        ...
        if DEBUG_BUILD:
            logging.debug(
                f"GPU能力检查结果: 是否可用={ability.is_capable}, 原因={ability.reason}"
            )

        return ability

    def _check_dynamic_memory(
        self,
        gpu_info: GPUInfo,
        model_name: str,
        context_length: int = None,
        ...
        if DEBUG_BUILD:
            logging.debug(
                f"动态显存检查: 可用={gpu_mem_gib:.2f}GiB, 总显存={total_mem_gib:.2f}GiB"
            )
        ...
        try:
            ...
            if context_length is None or n_parallel is None:
                ...
                if DEBUG_BUILD:
                    logging.debug(
                        f"使用推荐配置: 上下文长度={config['context_length']}, 并行数={config['n_parallel']}"
                    )
            else:
                config = {"context_length": context_length, "n_parallel": n_parallel}
                if DEBUG_BUILD:
                    logging.debug(
                        f"使用指定配置: 上下文长度={context_length}, 并行数={n_parallel}"
                    )
            ...
            if DEBUG_BUILD:
                logging.debug(
                    f"显存需求计算结果: 总需求={memory_usage['total_size_gib']:.2f}GiB, "
            ...
            # 如果计算失败，回退到基本显存检查
            if (
                gpu_mem_req_gib := model.minimal_gpu_memory_gib
            ) != 0 and gpu_mem_gib < gpu_mem_req_gib:
                return GPUAbility(
                    is_capable=False,
                    reason=f"显卡 {gpu_info.name} 的显存不足\n"
    ...
    def _check_static_memory(self, gpu_info: GPUInfo, model_name: str) -> GPUAbility:
        ...
        gpu_mem_gib = (
            math.ceil(BytesToGiB(gpu_mem)) if gpu_mem > (2**30) else BytesToGiB(gpu_mem)
        )
    ...
    def set_gpu_env(self, env, selected_gpu_display_name, selected_index):
        if DEBUG_BUILD:
            logging.debug(
                f"设置GPU环境变量: 显示名称={selected_gpu_display_name}, 索引={selected_index}"
            )

        # 从显示名称中找到对应的GPU key
        gpu_key = GPUDisplayHelper.find_gpu_key(
            selected_gpu_display_name, self.gpu_info_map
        )
        if not gpu_key:
        ...
        if gpu_info.gpu_type == GPUType.NVIDIA:
            ...
            if DEBUG_BUILD:
                logging.debug(
                    f"为NVIDIA GPU设置环境变量: CUDA_VISIBLE_DEVICES={selected_index}"
                )
        elif gpu_info.gpu_type == GPUType.AMD:
            ...
            if DEBUG_BUILD:
                logging.debug(
                    f"为AMD GPU设置环境变量: HIP_VISIBLE_DEVICES={env['HIP_VISIBLE_DEVICES']}"
                )
        elif gpu_info.gpu_type == GPUType.APPLE:
            env.pop("CUDA_VISIBLE_DEVICES", None)
            env.pop("HIP_VISIBLE_DEVICES", None)
            env["GGML_METAL"] = "1"
            logging.info("设置 GGML_METAL = 1")
            if DEBUG_BUILD:
                logging.debug("为Apple MPS设置环境变量: GGML_METAL=1")
        else:
